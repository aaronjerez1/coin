...
bool GetMyExternalIP2(const CAddress& addrConnect, const char* pszGet, const char* pszKeyword, unsigned int& ipRet)
{
    ...
    while (RecvLine(hSocket, strLine))
    {
        if (strLine.empty())
        {
            ...
            strLine = QString::fromStdString(strLine).trimmed().toStdString();
...
bool GetMyExternalIP(unsigned int& ipRet)
{
    ...
    const char* pszGet;       // Change to const char*
    const char* pszKeyword;   // Change to const char*
...
void ThreadOpenConnections2(void* parg)
{
    ...
    loop
    {
        ...
        // modified with range based for loop
        CRITICAL_BLOCK(cs_mapAddresses)
        {
            vIPC.reserve(mapAddresses.size());
            unsigned int nPrev = 0;
            for (const auto& item : mapAddresses) // Range-based for loop
            {
...
// void ThreadCoinMiner(void* parg)
// {
...
//     {
//         bool fRet = CoinMiner();
//         printf("CoinMiner returned %s\n\n\n", fRet ? "true" : "false");
//     }
//     CATCH_PRINT_EXCEPTION("CoinMiner()")
...
bool StopNode()
{
    ...
    if (vfThreadRunning[3]) printf("ThreadCoinMiner still running\n");

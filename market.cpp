...
// std::map<uint256, CProduct> mapMyProducts;




// std::map<uint256, CProduct> mapProducts;
...
//         std::pair<std::map<uint256, CProduct>::iterator, bool> item = mapProducts.insert(std::make_pair(hash, product));
...
// {
//     CReviewDB reviewdb;
//     std::map<uint256, vector<unsigned short> > pmapPropagate[2];
//     pmapPropagate[0][hashUserStart] = vAtoms;

//     for (int side = 0; !pmapPropagate[side].empty(); side = 1 - side)
//     {
//         std::map<uint256, vector<unsigned short> >& mapFrom = pmapPropagate[side];
//         std::map<uint256, vector<unsigned short> >& mapTo = pmapPropagate[1 - side];

//         for (std::map<uint256, vector<unsigned short> >::iterator mi = mapFrom.begin(); mi != mapFrom.end(); ++mi)
//         {
